...
public partial class MainWindowViewModel : ViewModelBase
{
    ...
    partial void OnSelectedNodeChanged(HunkFileTreeNode? value)
    {
        ...
        {
            var header = RecordParsers.ParseHunkHeader(headerRecord, IsBigEndian);
            if (header != null)
            {
        ...
        var texDataRecord = value.Records.FirstOrDefault(r => r.Type == HunkRecordType.TSETextureData || r.Type == HunkRecordType.TSETextureData2 || r.Type == HunkRecordType.TSETextureDataWii);
        
        if (texHeaderRecord != null && texDataRecord != null)
        {
            var header = RecordParsers.ParseTextureHeader(texHeaderRecord);
            if (header != null)
            {
                ...
                try
                {
                     ...
                     if (CurrentFontItems.Count > 0)
                     {
                         ...
                         // Get Header info from the first item (hack access via Parse? No, need access to Header)
                         // We don't have easy access to Header here, but we can look at the parsed values if we store them.
                         // For now, scan for non-empty.
                         ...
                         // Search for D-Pad candidates (approx 100-105px)
                         // And L-Stick (approx 87px)
                         ...
                         // Search for ANY glyph with Byte 2 == 0 (X=0)
                         var zeroByte2 = CurrentFontItems.FirstOrDefault(item => 
                         {
                             // We need to parse back hex or rely on our parsed Y/X?
                             // We parsed X = Data[2] * 4. So X == 0 means Data[2] == 0.
                             return item.X == 0;
                         });
    ...
    private Avalonia.Media.Imaging.Bitmap? DecodeTexture(TextureHeader header, byte[] data)
    {
        ...
        var ddsData = new byte[ddsHeader.Length + data.Length];
        Array.Copy(ddsHeader, 0, ddsData, 0, ddsHeader.Length);
        Array.Copy(data, 0, ddsData, ddsHeader.Length, data.Length);
        ...
        {
            Marshal.Copy(image.Data, 0, buffer.Address, image.DataLen);
        }
    ...
    public async Task OpenFile(IStorageProvider storageProvider)
    {
        ...
        if (files.Count >= 1)
        {
            ...
            try 
            {
                var records = _parser.Parse(path, IsBigEndian);
